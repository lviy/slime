...
class RayTrainGroup:
    ...
    def onload(self, reason: str = "unspecified"):
        return ray.get([actor.wake_up.remote(reason=reason) for actor in self._actor_handlers])

    def offload(self, reason: str = "unspecified"):
        return ray.get([actor.sleep.remote(reason=reason) for actor in self._actor_handlers])
