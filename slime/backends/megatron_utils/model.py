...
def forward_only(
    ...
    def forward_step(
        ...
        if breakdown_enabled:
            ...
            kernel_start_time = torch.cuda.Event(enable_timing=True) if torch.cuda.is_available() else None
            kernel_end_time = torch.cuda.Event(enable_timing=True) if torch.cuda.is_available() else None
            if kernel_start_time is not None:
        ...
        if breakdown_enabled and kernel_start_time is not None and kernel_end_time is not None:
            kernel_end_time.record()
            _cuda_sync_if_needed()
            elapsed_s = kernel_start_time.elapsed_time(kernel_end_time) / 1000.0
            Timer().add(f"{profile_timer_prefix}_kernel", elapsed_s)
            logger.info(
                "[PTMProfile] stage=%s component=kernel ptm_enabled=%s applied=%s elapsed_ms=%.3f",
