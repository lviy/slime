...
# Clean up previous local Ray / SGLang state for a fresh rollout-only run.
pkill -9 sglang || true
ray stop --force || true
pkill -9 ray || true
pkill -9 redis || true
...
export SLIME_PTM_ROLLOUT_MODEL_PATH="${SLIME_PTM_ROLLOUT_MODEL_PATH:-${SLIME_PTM_ROLLOUT_BASE_DIR}/Qwen3-8B}"
export SLIME_PTM_ROLLOUT_REF_PATH="${SLIME_PTM_ROLLOUT_REF_PATH:-${SLIME_PTM_ROLLOUT_BASE_DIR}/Qwen3-8B_torch_dist}"
...
export SLIME_PTM_ROLLOUT_DUMP_DIR="${SLIME_PTM_ROLLOUT_DUMP_DIR:-/tmp/qwen3_8b_ptm_rollout}"
