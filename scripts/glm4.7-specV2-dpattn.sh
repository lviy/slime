#!/bin/bash
# =============================================================================
# GLM-4.7-Flash-0309 4节点训练脚本 (稳定版)
# 
# 基于 3/14 四机实验失败分析创建：
#   - 降低 sglang-mem-fraction-static: 0.85 → 0.78 (防止 cuBLAS workspace OOM)
#   - 降低 sglang-cuda-graph-max-bs: 8 → 4 (减少 CUDA graph 显存占用)
#   - 降低 sglang-max-running-requests: 32 → 16 (降低并发推理显存峰值)
#   - 降低 sglang-server-concurrency: 512 → 128 (减少并发请求压力)
#   - 确保包含 --sglang-dp-size 4 / --sglang-enable-dp-attention / --sglang-enable-dp-lm-head
#   - 保留 PYTORCH_ALLOC_CONF=expandable_segments:True (减少显存碎片)
# =============================================================================
...
# 模型路径
# ========================
export model_load_path=${model_load_path:-"/gfs/space/chatrl/public/models/ZhipuAI/GLM-4.7-Flash-0309"}
export torch_dist_model_load_path=${torch_dist_model_load_path:-"/gfs/space/chatrl/public/models/ZhipuAI/GLM-4.7-Flash-0309-torch-dist"}

export root_dir="/gfs/space/chatrl/users/hxh/code/slime_latest"
export save_dir="/gfs/space/chatrl/users/hxh/models/slime_models"
export DEBUG_SGLANG_ROOT="${root_dir}/third_party/sglang_pod_20260322"
export RUNTIME_LOG_DIR="${root_dir}/runtime_logs"
mkdir -p "${save_dir}" "${RUNTIME_LOG_DIR}"
...
export train_prompt_batch_size=${train_prompt_batch_size:-32}  # 每步更新用多少条 prompt
export n_samples=${n_samples:-16}
export global_batch_size=$(( train_prompt_batch_size * n_samples ))   # = 32 * 8 = 256 responses
export num_steps_per_rollout=$(( rollout_batch_size / train_prompt_batch_size ))  # = 128 / 32 = 4 步

# export global_batch_size=${global_batch_size:-32}
# export mini_batch_size=${mini_batch_size:-16}
# export num_steps_per_rollout=$(( rollout_batch_size / mini_batch_size ))

export eval_interval=${eval_interval:-4}
export over_sampling_batch_size=${over_sampling_batch_size:-4096}
...
if [ "$train_from_resume" = "True" ]; then
    ...
    export model_save_path=${save_dir}/glm4.7-flash-dapo/${model_save_name}
else
   ...
   export model_save_path=${save_dir}/glm4.7-flash-dapo/${model_save_name}-${model_suffix_name}
fi
echo "✅ Model will be saved to: ${model_save_path}"

export TENSORBOARD_DIR=${model_save_path}
export LOG_FILE=${model_save_path}/train_log_$(date +%Y-%m-%d_%H-%M-%S).log
...
# ==============================================================
# Megatron 并行策略 (4节点 32卡)
# TP=4, EP=8, DP = 32 / 4 = 8 (匹配 EP=8)
# ==============================================================
PERF_ARGS=(
...
# ==============================================================
# SGLang 配置 — 稳定版（3/14 失败分析修复）
#
# 32 GPU / 8 (gpus-per-engine) = 4 engines
# dp-size = 4, attn_tp_size = 8/4 = 2, num_heads=20, 20%2=0 ✅
#
# 关键变更（与旧版对比）:
#   mem-fraction-static:    0.85 → 0.78  (为 cuBLAS workspace 留更多空间)
#   cuda-graph-max-bs:      8    → 4     (减少 CUDA graph cache 占用)
#   max-running-requests:   32   → 16    (降低并发推理显存峰值)
#   server-concurrency:     512  → 128   (减少并发请求压力)
# ==============================================================
SGLANG_ARGS=(
...
# ============================================================
# 动态节点启动 (ACTOR_NUM_NODES nodes × 8 GPU)
# ============================================================
...
if [ "$RANK" -eq 0 ]; then
   ...
   ray start --address="${MASTER_ADDR}:6379" --num-gpus 8 --disable-usage-stats --block
fi
