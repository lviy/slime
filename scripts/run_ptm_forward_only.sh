...
if [[ $# -eq 0 && -z "${SLIME_PTM_FORWARD_ONLY_ROLLOUT_PT:-}" ]]; then
  cat >&2 <<'EOF'
Usage:
  scripts/run_ptm_forward_only.sh --rollout-pt /path/to/rollout_0.pt [extra args]
  scripts/run_ptm_forward_only.sh --rollout-pt '/path/to/rollout_{rollout_id}.pt' --num-rollout 2

Environment fallback:
  SLIME_PTM_FORWARD_ONLY_ROLLOUT_PT=/path/to/rollout_0.pt scripts/run_ptm_forward_only.sh [extra args]
EOF
  exit 2
fi

declare -a cmd=(python3 tests/test_ptm_forward_only.py)
if [[ -n "${SLIME_PTM_FORWARD_ONLY_ROLLOUT_PT:-}" ]]; then
  cmd+=(--rollout-pt "${SLIME_PTM_FORWARD_ONLY_ROLLOUT_PT}")
fi
cmd+=("$@")
