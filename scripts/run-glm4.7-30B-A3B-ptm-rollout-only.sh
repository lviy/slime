...
export SLIME_PTM_ROLLOUT_BASE_DIR="${SLIME_PTM_ROLLOUT_BASE_DIR:-${BASE_DIR:-/root/models}}"
export SLIME_PTM_ROLLOUT_MODEL_PATH="${SLIME_PTM_ROLLOUT_MODEL_PATH:-${SLIME_PTM_ROLLOUT_BASE_DIR}/GLM-4.7-Flash}"
export SLIME_PTM_ROLLOUT_REF_PATH="${SLIME_PTM_ROLLOUT_REF_PATH:-${SLIME_PTM_ROLLOUT_BASE_DIR}/GLM-4.7-Flash_torch_dist/}"
...
export SLIME_PTM_ROLLOUT_DUMP_DIR="${SLIME_PTM_ROLLOUT_DUMP_DIR:-/tmp/glm47_flash_ptm_rollout}"
...
export SLIME_PTM_ROLLOUT_NUM_GPUS="${SLIME_PTM_ROLLOUT_NUM_GPUS:-8}"
export SLIME_PTM_ROLLOUT_DP_SIZE="${SLIME_PTM_ROLLOUT_DP_SIZE:-${SLIME_PTM_ROLLOUT_NUM_GPUS}}"
export SLIME_PTM_ROLLOUT_BATCH_SIZE="${SLIME_PTM_ROLLOUT_BATCH_SIZE:-4}"
export SLIME_PTM_ROLLOUT_NUM_SAMPLES="${SLIME_PTM_ROLLOUT_NUM_SAMPLES:-16}"
export SLIME_PTM_ROLLOUT_MAX_RESPONSE_LEN="${SLIME_PTM_ROLLOUT_MAX_RESPONSE_LEN:-2048}"
export SLIME_PTM_ROLLOUT_MAX_TOKENS_PER_GPU="${SLIME_PTM_ROLLOUT_MAX_TOKENS_PER_GPU:-32768}"
export SLIME_PTM_ROLLOUT_MEM_FRACTION="${SLIME_PTM_ROLLOUT_MEM_FRACTION:-0.7}"
export SLIME_PTM_ROLLOUT_CUDA_GRAPH_MAX_BS="${SLIME_PTM_ROLLOUT_CUDA_GRAPH_MAX_BS:-64}"
export SLIME_PTM_ROLLOUT_SEED="${SLIME_PTM_ROLLOUT_SEED:-42}"
export SLIME_PTM_ROLLOUT_TEMPERATURE="${SLIME_PTM_ROLLOUT_TEMPERATURE:-1.0}"
...
SGLANG_ARGS=(
  ...
  --rollout-num-gpus-per-engine "${SLIME_PTM_ROLLOUT_NUM_GPUS}"
  --sglang-mem-fraction-static "${SLIME_PTM_ROLLOUT_MEM_FRACTION}"
  --sglang-enable-dp-attention
  --sglang-dp-size "${SLIME_PTM_ROLLOUT_DP_SIZE}"
